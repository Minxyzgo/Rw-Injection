    ...
    override fun process(elements: MutableSet<out TypeElement>, env: RoundEnvironment): Boolean {
        ...
        allData.forEach { (name, type, declaringClassName, rename, args) ->
            ...
            if(type == RenameType.Field) {
                ...
                clazz.addMethod(CtMethod.make("@rwij.Proxy public ${field.type.name} get$rename() { return this.$name; }", clazz))
                clazz.addMethod(CtMethod.make("@rwij.Proxy public void set$rename(${field.type.name} p) { this.$name = p; }", clazz))
